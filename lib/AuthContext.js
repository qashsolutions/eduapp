...
export function AuthProvider({ children }) {
  ...
  const checkSession = async () => {
    try {
      // First check for student session in sessionStorage
      const studentData = sessionStorage.getItem('studentData');
      if (studentData) {
        const student = JSON.parse(studentData);
        // Load full student profile from database
        await loadUserProfile(student.id);
        return;
      }
  ...
  const loadUserProfile = async (userId) => {
    try {
      ...
      if (studentData && JSON.parse(studentData).id === userId) {
        ...
        const response = await fetch('/api/get-student-profile', {
          ...
          headers: { 
            ...
            'Authorization': `Student ${JSON.parse(studentData).sessionToken}`
          },
