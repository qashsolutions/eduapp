...
export const createUser = async (email, userId, role = 'student', grade = null, isParent = false, parentInfo = null) => {
  try {
    ...
    if (role === 'student' && parentInfo && !isParent) {
      ...
      // Send approval email to parent using Firebase
      try {
        ...
        // Send email via Firebase Auth (custom email action)
        // Since Firebase doesn't have a direct parent approval flow, 
        // we'll trigger this through our API
        const response = await fetch('/api/send-parent-approval', {
          method: 'POST',
