...
export const createQuestionPrompt = (topic, difficulty, grade, context, subtopic) => {
...
IMPORTANT: Return ONLY valid JSON, no additional text or formatting.`;
    ...
    const response = await openai.responses.create({
      model: "gpt-4-0613",
      instructions: "You are an expert educational content creator specializing in K-12 curriculum. Always return responses in valid JSON format.",
      input: jsonPrompt
    });
...
export const generateWithClaude = async (anthropic, prompt) => {
  try {
    // Add JSON instruction to the prompt
    const jsonPrompt = `You are an expert educational content creator. ${prompt}
...
export const generateSocraticFollowup = async (aiClient, topic, question, wrongAnswer, difficulty) => {
  ...
  try {
    if (aiModel === 'openai') {
      const response = await aiClient.responses.create({
        model: "gpt-4-0613",
        instructions: "You are a supportive teacher using the Socratic method.",
        input: prompt
      });
