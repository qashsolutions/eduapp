...
${isComprehension ? `READING COMPREHENSION FORMAT:
1. Create a passage of **exactly** ${minWords}-${minWords + 50} words about ${subtopic}
2. The passage MUST be complete, engaging, and age-appropriate for grade ${grade}
3. Write ONE multiple choice question that tests comprehension of the passage
4. The question MUST reference the passage (use words like "according to the passage", "the author states", "the main idea", etc.)
5. Create four answer choices (A, B, C, D) where only ONE is correct based on the passage
6. Wrong answers should be plausible but clearly incorrect when compared to the passage` : 
`EDUCATIONAL QUESTION FORMAT:
1. Create a detailed context/problem setup (minimum ${Math.min(minWords, 100)} words)
2. Write ONE clear question based on the context
3. Create four answer choices (A, B, C, D) with only one correct answer
4. Wrong answers should be plausible but clearly incorrect`}

CRITICAL REQUIREMENTS:
- Content must be appropriate for K-12 education (no profanity, violence, inappropriate themes)
- Align with ISEE standards for grade ${grade}
- The passage/context is MANDATORY - not optional

Return JSON format:
{
  "question": "Question that directly references the passage/context",
  "context": "The complete ${minWords}+ word passage or detailed problem context - REQUIRED",
  "options": {
    "A": "First option",
    "B": "Second option", 
    "C": "Third option",
