...
`;
  ...
  return `Generate a ${isComprehension ? 'reading comprehension exercise' : 'educational question'} for a grade ${grade} student.
...
3. Write ONE question that can ONLY be answered by reading the ${isComprehension ? 'passage' : 'context'}
4. Create 4 answer options (A, B, C, D) where ONLY ONE is 100% correct based on the content
5. Wrong answers must be plausible but clearly incorrect when compared to the ${isComprehension ? 'passage' : 'context'}
6. Question must use phrases like "${isComprehension ? 'According to the passage' : 'Based on the information'}", "The ${isComprehension ? 'author states' : 'text explains'}", "${isComprehension ? 'The main idea' : 'The problem shows'}"
7. Content must be appropriate for K-12 education (no profanity, violence, inappropriate themes)
8. Align with ISEE standards for grade ${grade}

EXAMPLE STRUCTURE:
{
  "question": "${isComprehension ? 'According to the passage' : 'Based on the information'}, what is [specific detail from the ${isComprehension ? 'passage' : 'context'}]?",
  "context": "[WRITE THE ACTUAL ${minWords}+ WORD ${isComprehension ? 'PASSAGE' : 'CONTEXT'} HERE - NOT '${context}' but real content about ${subtopic}]",
  "options": {
    "A": "Answer that seems right but contradicts the ${isComprehension ? 'passage' : 'context'}",
    "B": "The exact correct answer found in the ${isComprehension ? 'passage' : 'context'}", 
    "C": "Plausible but not mentioned in ${isComprehension ? 'passage' : 'context'}",
    "D": "Reasonable but ${isComprehension ? 'passage' : 'context'} says opposite"
  },
  "correct": "B",
  "explanation": "The ${isComprehension ? 'passage' : 'context'} clearly states [quote that proves B is correct]"
}

REMEMBER: The context field must contain the actual ${isComprehension ? 'reading passage' : 'problem content'} - NOT the context type "${context}".
...
export const generateWithOpenAI = async (openai, prompt) => {
  ...
  try {
    ...
    try {
      ...
      console.log('[OpenAI] Successfully parsed JSON. Context length:', parsedContent.context?.length || 0);
      console.log('[OpenAI] Question:', parsedContent.question?.substring(0, 100) + '...');
      ...
      if (jsonMatch) {
        ...
        console.log('[OpenAI] Extracted JSON. Context length:', parsedContent.context?.length || 0);
        return sanitizeContent(parsedContent);
      }
...
IMPORTANT: Return ONLY valid JSON, no additional text or formatting.`;
    ...
    console.log('[Claude] Context length:', parsedContent.context?.length || 0);
...
export const validateQuestion = (question, topic = null, grade = 8) => {
  ...
  // Enhanced validation for reading comprehension topics (temporarily relaxed)
